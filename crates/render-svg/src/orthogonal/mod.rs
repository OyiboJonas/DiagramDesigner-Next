...
use next_domain::{
    Color, Connector, Document, Element, ElementId, ElementKind, ElementStyle, Endpoint, LineStyle,
    MarkerStyle, Point, Port, Rect,
};
...
pub(super) fn apply_orthogonal_connectors(
    ...
    // The Phase-1 core intentionally skips this primitive. Work backwards so a
    // later orthogonal fragment is already present when we insert an earlier one;
    // this reconstructs render-plan order without introducing SVG state upstream.
    for index in (0..plan.items.len()).rev() {
        ...
        let directions = resolve_end_directions(document, connector);
...
fn resolve_end_directions(
    ...
    // Delphi object memory initializes the enum to its first value. Upstream then
...
fn resolve_endpoint_direction(
    ...
    // Legacy TAxisLineObject treats another axis connector like a free endpoint.
    ...
    let Some(port) = target
        .ports
        .iter()
        .find(|port| port.id == connection.port_id)
    else {
        return free_end_direction(connector, endpoint);
    };
...
fn build_route(
    ...
    // Preserve every non-zero DrawLineStyle segment boundary from the upstream
    // route. In particular, the PCenter split must remain for dashed styles and a
    // same-axis same-direction hairpin must not collapse into a direct segment.
    points.dedup_by(|left, right| approx_eq(left.x, right.x) && approx_eq(left.y, right.y));

    let marker_path_d = sharp_path(&points);
    let use_rounded_corners =
        matches!(connector.line_style, LineStyle::Solid | LineStyle::Outline) && radius > 0.0;
...
fn rounded_path(points: &[Point], radius_x: f64, radius_y: f64) -> String {
    ...
    for index in 1..points.len() - 1 {
        ...
        let incoming_radius = if incoming.0 != 0 {
            radius_x.min(incoming_length)
        } else {
            radius_y.min(incoming_length)
        };
        let outgoing_radius = if outgoing.0 != 0 {
        ...
        let before = Point {
            x: corner.x - f64::from(incoming.0) * incoming_radius,
            y: corner.y - f64::from(incoming.1) * incoming_radius,
        };
        let after = Point {
            x: corner.x + f64::from(outgoing.0) * outgoing_radius,
            y: corner.y + f64::from(outgoing.1) * outgoing_radius,
        };
        let arc_rx = if incoming.0 != 0 {
            incoming_radius
        } else {
            outgoing_radius
        };
        let arc_ry = if incoming.1 != 0 {
            incoming_radius
        } else {
            outgoing_radius
        };
        let cross = i16::from(incoming.0) * i16::from(outgoing.1)
            - i16::from(incoming.1) * i16::from(outgoing.0);
        let sweep = i32::from(cross > 0);
        write!(
            result,
            " L {} {} A {} {} 0 0 {} {} {}",
            num(before.x),
...
fn unit_axis(from: Point, to: Point) -> (i8, i8) {
    let dx = to.x - from.x;
    let dy = to.y - from.y;
...
fn render_connector_group(
    ...
    if connector.start_marker != MarkerStyle::None || connector.end_marker != MarkerStyle::None {
        write!(
            ...
            element.id.0, route.marker_path_d,
        )
